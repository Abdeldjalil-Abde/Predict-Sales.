// ============================================================
//  SalesCast AI — main.js
//  Front-end controller.
//
//  All ML training now runs server-side (Python / ml_engine.py).
//  This file handles:
//    • Navigation & page switching
//    • File upload & CSV parsing (PapaParse)
//    • Sample-data generation
//    • Calling  POST /api/train  and  POST /api/predict
//    • Rendering Chart.js visualisations from the API response
//    • Auth (demo + OAuth + logout)
...
async function startTraining() {
  ...
  try {
    addLog(
      "⚙️  Server is training models (Python / scikit-learn + TensorFlow)…",
      "info",
    );
    ...
    // ── Store results directly (already in correct format)
...
function renderResults(results) {
  ...
  // Destroy old charts
  Object.values(trainCharts).forEach((c) => {
    try {
      c.destroy();
    } catch (e) {}
  });
  ...
  // ── Comparison bar chart  (R² + Accuracy)
  ...
  trainCharts.met = new Chart(document.getElementById("metricsChart"), {
    ...
    data: {
      ...
      datasets: [
        {
          label: "MAE",
          data: keys.map((k) => results[k].test.mae),
          backgroundColor: "#4f8ef7cc",
          borderRadius: 6,
        },
        {
          label: "RMSE",
          data: keys.map((k) => results[k].test.rmse),
          backgroundColor: "#7c5cfc99",
          borderRadius: 6,
        },
      ],
    },
  ...
  // ── Feature importance  (prefer DT, else first available)
  ...
  trainCharts.feat = new Chart(document.getElementById("featureChart"), {
    ...
    data: {
      ...
      datasets: [
        {
          label: "Importance",
          data: sorted.map((s) => Math.round(s.v * 1000) / 1000),
  ...
  trainCharts.split = new Chart(document.getElementById("splitChart"), {
    ...
    data: {
      ...
      datasets: [
        {
          label: "Train R²",
          data: keys.map((k) => results[k].train.r2),
...
function switchChart(id, btn) {
  ["comparison", "predictions", "metrics"].forEach((c) => {
    document.getElementById("chart-" + c).style.display =
      c === id ? "block" : "none";
  });
  document
...
function downloadCSV() {
  ...
  for (let i = 0; i < n; i++) {
    rows.push([
      ...
      ...keys.map((k) =>
        Math.round(
          Math.abs(state.results[k].actuals[i] - state.results[k].predictions[i]),
        ),
      ),
    ]);
  }
...
window.addEventListener("load", async () => {
  try {
    ...
    /* backend not running — stay on login page */
  }
});
...
function applyUser(user) {
  ...
  const initial =
    user.initial || (user.name ? user.name[0].toUpperCase() : "?");
...
async function doLogin() {
  ...
  btn.disabled = true;
  btn.innerHTML =
    '<span class="spinner" style="border-top-color:#fff;border-color:rgba(255,255,255,.3)"></span> Signing in…';
  ...
  try {
    ...
    if (username === "demo" && password === "demo123") {
      ...
      showLoginError(
        "Cannot reach server. For demo mode use <b>demo</b> / <b>demo123</b>",
      );
      btn.disabled = false;
      btn.textContent = "Sign In";
    }
